"""cross_instruction_extract_dense.py — Dense L0-L8 cross-instruction feature extraction.

Purpose: The original cross-instruction extraction (instruction_conditional_extract.py) samples
layers [0, 8, 16, 24, 31]. We found instruction encoding jumps from 10% (L0) to 100% (L8).
This script extracts all intermediate layers L0-L8 (9 layers) to pinpoint exactly where
instruction routing emerges.

Output: HDF5 with structure:
  /task_{tid}/frame_{fid}/instruction_{iid}/{image_mean, last_preaction} shape (9, 4096)
  /metadata/{instructions, layers, task_files}
  f.attrs['layers'] = [0,1,2,3,4,5,6,7,8]

Expected runtime: ~4-5 min per condition (same 5000 forward passes, hooking 9 vs 5 layers
is negligible since all layers are computed in every forward pass anyway).
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Dense L0-L8 cross-instruction feature extraction"
    )
...
def extract_features_single(model, processor, image, instruction, device):
    """Forward pass for one (image, instruction) pair. Returns (image_mean, last_preaction) each shape (9, 4096)."""
...
def main():
    ...
    output_filename = f"{args.condition}_cross_inst_dense.h5"
    output_path = os.path.join(args.output_dir, output_filename)

    print(f"=== Dense L0-L8 Cross-Instruction Feature Extraction ===")
