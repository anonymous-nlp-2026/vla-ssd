...
def install_ablation_hook(model, heads_to_ablate):
    ...
    def ablated_forward(
        ...
        **kwargs,
        ...
        if hasattr(self_attn, "_ablate_heads") and self_attn._ablate_heads is not None:
            kv_len = attn_weights.shape[-1]
            if INST_START < kv_len:
                attn_weights[:, self_attn._ablate_heads, :, INST_START:] = 0.0
...
def main():
    ...
    original_eager_fn = install_ablation_hook(model, CONTROL_HEADS_8)
        ...
        with h5py.File(hdf5_path, "r") as f:
            for demo_idx in range(n_demos):
                ...
                for step_idx in range(n_steps):
                    ...
                    # Ablated
                    layer1_attn._ablate_heads = CONTROL_HEADS_8
    ...
    output = {
        "config": {
            ...
            "model": "openvla-7b",
            "ablation": "L1_non_instruction_8_heads_zeroed",
            "ablated_heads": CONTROL_HEADS_8,
    ...
    out_path = os.path.join(RESULTS_DIR, "action_trajectory_control.json")
