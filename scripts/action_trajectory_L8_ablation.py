"""Open-loop action trajectory analysis: baseline vs L8-full-ablation on LIBERO demo observations."""
...
# ── Ablation hook (monkey-patch LlamaAttention.forward for transformers 4.40.1) ─
def install_ablation_hook(model):
    import math
    from transformers.models.llama.modeling_llama import repeat_kv, apply_rotary_pos_emb

    layer_attn = model.language_model.model.layers[LAYER_IDX].self_attn
    layer_attn._ablate_active = False
    original_forward = layer_attn.forward.__func__ if hasattr(layer_attn.forward, '__func__') else layer_attn.forward
    ...
    def ablated_forward(
        ...
        **kwargs,
        ...
        past_key_value = getattr(self_attn, "past_key_value", past_key_value)
        ...
        if hasattr(self_attn, "_ablate_active") and self_attn._ablate_active:
    ...
    import types
    layer_attn.forward = types.MethodType(ablated_forward, layer_attn)
...
def remove_ablation_hook(model, original_fn):
    import types
    layer_attn = model.language_model.model.layers[LAYER_IDX].self_attn
    layer_attn.forward = types.MethodType(original_fn, layer_attn)
    layer_attn._ablate_active = False
...
def main():
    ...
    original_forward_fn = install_ablation_hook(model)
    layer_attn = model.language_model.model.layers[LAYER_IDX].self_attn
    ...
    print(f"Ablation: L{LAYER_IDX} all {len(ALL_HEADS)} heads, zero attention output")
    ...
    for task_idx, hdf5_path in enumerate(task_files):
        ...
        with h5py.File(hdf5_path, "r") as f:
            for demo_idx in range(n_demos):
                ...
                for step_idx in range(n_steps):
                    ...
                    layer_attn._ablate_active = True
                    ...
                    if total_passes % 100 == 0:
                        ...
                        print(
                            f"  [{total_passes}/{n_tasks*n_demos*n_timesteps*2}] "
                            f"{rate:.1f} passes/s, ETA {eta/60:.1f}min"
                        )
        ...
        task_metrics = compute_task_metrics(task_b, task_a, task_b_ids, task_a_ids, task_kls)
        per_task_data[task_name] = task_metrics

        print(
            f"  Task MSE: {task_metrics['paired_mse']['mean']:.6f} "
            f"(d={task_metrics['paired_mse']['cohens_d']:.3f}), "
            f"token_div={task_metrics['token_divergence_rate']:.4f}, "
            f"KL={task_metrics['kl_divergence']['mean']:.4f}"
        )
    ...
    # ── Global metrics ──────────────────────────────────────────────────────
    layer_attn._ablate_active = False
    remove_ablation_hook(model, original_forward_fn)
    ...
    output = {
        "config": {
            ...
            "model": "openvla-7b",
            "ablation": f"L{LAYER_IDX}_all_32_heads_zero_attn_output",
    ...
    out_path = os.path.join(RESULTS_DIR, "action_trajectory_L8_ablation.json")
