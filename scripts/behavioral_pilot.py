"""Behavioral pilot: compare OpenVLA baseline vs L1-ablated on LIBERO-Goal tasks."""
...
# ── Manual autoregressive generation (bypasses generate() DynamicCache issues) ──
...
def main():
    ...
    for task_id in TASK_IDS:
        ...
        for condition in CONDITIONS:
            ...
            for ep in range(N_EPISODES):
                ...
                print(f"    ep {ep:2d}: {status} | R={cumulative_reward:.3f} | steps={step_i+1} | {ep_time:.1f}s")

            sr = sum(e["success"] for e in episodes) / len(episodes)
            avg_r = np.mean([e["reward"] for e in episodes])
            print(f"  >> {condition} SR={sr:.2f}, avg_reward={avg_r:.3f}")
    ...
    summary = {
        ...
        "ablated_success_rate": ablated_successes / total,
        "delta": (baseline_successes - ablated_successes) / total,
        "per_task": per_task,
    }
    ...
    out_path = os.path.join(RESULTS_DIR, "behavioral_pilot.json")
