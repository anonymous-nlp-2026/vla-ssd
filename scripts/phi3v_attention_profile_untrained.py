"""Attention profile for untrained (base Phi-3 LLM) in TracVLA-Phi3V architecture."""
...
RESULT_PATH = "./results/attention/phi3v_untrained_attention_profile.json"
...
def main():
    ...
    print("Loading untrained (base Phi-3 LLM) hybrid model...")
    ...
    for hf in hdf5_files:
        ...
        with h5py.File(str(hf), "r") as f:
            ...
            img_arr = demo["obs"]["agentview_rgb"][0]
    ...
    if dry_run:
        ...
        print(f"input_ids len={len(ids)}, image tokens=[{img_s}:{img_e}] = {img_e - img_s} tokens")
        inputs = {k: v.to(device) for k, v in inputs.items()}
        if "pixel_values" in inputs and inputs["pixel_values"].dtype == torch.float32:
            inputs["pixel_values"] = inputs["pixel_values"].to(torch.bfloat16)
        outputs = model(**inputs, output_attentions=True)
    ...
    compat_path = os.path.join(OUTPUT_DIR, "full_32layer_phi3v_untrained.json")
    ...
    print("\n" + "=" * 60)
    print("Phi3V UNTRAINED (base Phi-3 LLM) Attention Profile")
    print("=" * 60)
