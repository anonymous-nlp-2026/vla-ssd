"""TracVLA-Phi3V per-layer action R² probe.
...
OUT_JSON = "./results/functional_validation/phi3v_r2_action_probe.json"
...
def load_phi3v_model(device_str):
    ...
    print(f"Model loaded: {n_params:.1f}B params on {device_str}")
...
def run_extraction(device):
    ...
    print(f"\nLoading TracVLA-Phi3V for feature extraction...")
    model, processor = load_phi3v_model(device)
...
def main():
    ...
    print("=" * 60)
    print("TracVLA-Phi3V Per-Layer Action R² Probe")
