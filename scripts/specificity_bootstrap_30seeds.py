"""
Specificity bootstrap: 30-seed random-24-head ablation at Layer 1.

Two-phase operation:
  --generate-cache: GPU phase. Load model, forward pass, save intermediates to disk.
  (default):        CPU phase. Load cache, run 30 random seeds + dominant-24, statistics.

Usage:
  # Phase 1: Generate cache (needs GPU, ~20 min)
  python specificity_bootstrap_30seeds.py --generate-cache

  # Phase 2: Bootstrap (CPU only, ~1-2 hours)
  CUDA_VISIBLE_DEVICES="" python specificity_bootstrap_30seeds.py
"""
...
CACHE_PATH = "./results/l1_head_ablation/l1_attention_cache.pt"
OUTPUT_PATH = "./results/l1_head_ablation/specificity_bootstrap_30seeds.json"
...
# =============================================================================
# Phase 1: GPU caching (only used with --generate-cache)
# =============================================================================
...
def generate_cache(args):
    ...
    print("=" * 60)
    print("Phase 1: Generating attention cache (GPU)")
    print("=" * 60)
    ...
    for task_idx, (hdf5_path, task_name) in enumerate(zip(hdf5_files, task_names)):
        ...
        data = f["data"]
        demo_keys = sorted(data.keys(), key=lambda x: int(x.split("_")[1]))[:args.demos_per_task]
    ...
    elapsed = time.time() - t0
    print(f"\nCache saved to {args.cache_path}")
    print(f"  {total_frames} frames, {len(task_names)} tasks, {len(task_boundaries)} demos")
    print(f"  Took {elapsed/60:.1f} min")


# =============================================================================
# Phase 2+3: CPU-only bootstrap
# =============================================================================
...
def reconstruct_l1_features_cpu(cache, weights, ablate_heads, chunk_size=4096):
    ...
    w_o = weights["o_proj"].float()
    w_gate = weights["gate_proj"].float()
    w_up = weights["up_proj"].float()
    w_down = weights["down_proj"].float()
    norm_w = weights["post_attn_norm_weight"].float()
    ...
    for start in range(0, N, chunk_size):
        ...
        ctx = full_ctx[start:end].clone().float()
        if ablate_heads is not None and len(ablate_heads) > 0:
            heads = list(ablate_heads)
            ns = non_inst_s[start:end, heads, :].float()
            nw = non_inst_w[start:end, heads].unsqueeze(-1).float()
            nw = torch.clamp(nw, min=1e-9)
            ctx[:, heads, :] = ns / nw

        concat_ctx = ctx.reshape(end - start, HIDDEN_SIZE)
        res = residual[start:end].float()
...
def run_bootstrap(args):
    print("=" * 60)
    print(f"Specificity Bootstrap: {args.n_random} seeds (CPU)")
    print("=" * 60)

    if not os.path.exists(args.cache_path):
        print(f"\nERROR: Cache not found at {args.cache_path}")
        print("Run with --generate-cache first (needs GPU).")
        return

    print(f"\nLoading cache from {args.cache_path}...")
    t0 = time.time()
    cache = torch.load(args.cache_path, map_location="cpu", weights_only=False)
    weights = cache["layer1_weights"]
    print(f"  Loaded in {time.time()-t0:.1f}s")
    print(f"  {cache['full_context'].shape[0]} frames, {len(cache['task_names'])} tasks")

    # --- Baseline ---
    print("\n  Computing baseline (no ablation)...")
    t1 = time.time()
    baseline_features = reconstruct_l1_features_cpu(cache, weights, ablate_heads=None)
    print(f"    Reconstruction: {time.time()-t1:.1f}s")
    ...
    t1 = time.time()
    dom_features = reconstruct_l1_features_cpu(cache, weights, ablate_heads=INSTRUCTION_HEADS_24)
    print(f"    Reconstruction: {time.time()-t1:.1f}s")
    ...
    for i in range(args.n_random):
        ...
        t1 = time.time()
        rand_features = reconstruct_l1_features_cpu(cache, weights, ablate_heads=random_heads)
    ...
    n = len(increases)
    se = random_sd / np.sqrt(n)
    t_crit = stats.t.ppf(0.975, df=n-1)
    ci_lo = random_mean - t_crit * se
    ci_hi = random_mean + t_crit * se

    bootstrap_cis = []
    rng_boot = np.random.RandomState(SEED)
    for _ in range(10000):
        sample = rng_boot.choice(increases, size=n, replace=True)
        bootstrap_cis.append(np.mean(sample))
    boot_ci_lo = float(np.percentile(bootstrap_cis, 2.5))
    boot_ci_hi = float(np.percentile(bootstrap_cis, 97.5))
    ...
    print(f"  Random-24 CI (t):      [{ci_lo:.4f}, {ci_hi:.4f}]")
    print(f"  Random-24 CI (boot):   [{boot_ci_lo:.4f}, {boot_ci_hi:.4f}]")
    ...
    output = {
        ...
        "statistics": {
            ...
            "ci_95_t": [round(ci_lo, 4), round(ci_hi, 4)],
            "ci_95_bootstrap": [round(boot_ci_lo, 4), round(boot_ci_hi, 4)],
        ...
        "metadata": {
            ...
            "reconstruction": "cached attn intermediates + layer1 recomputation (CPU float32)",
        }
    }
    ...
    ax.hist(increases, bins=15, color="#4C72B0", edgecolor="white", alpha=0.8, label="Random-24 subsets")
    ...
    ax.axvspan(boot_ci_lo, boot_ci_hi, alpha=0.15, color="gray", label="95% CI (bootstrap)")
    ax.set_xlabel(r"R$^2$ increase (ablated $-$ baseline)")
    ax.set_ylabel("Count")
    ax.set_title(f"Random-Subset Ablation (n={args.n_random})\n"
                 f"z={z_score:.2f}, percentile={percentile:.0f}%")
    ax.legend(fontsize=8)
    plt.tight_layout()

    pdf_path = os.path.join(args.fig_dir, "fig_specificity_bootstrap_30seeds.pdf")
    png_path = os.path.join(args.fig_dir, "fig_specificity_bootstrap_30seeds.png")
